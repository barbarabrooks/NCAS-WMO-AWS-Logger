...
class rhtTCPListener(protocol.Protocol):
    """This is a very basic TCP listener for RH & T sensor"""

    outpath = "/home/pi/DATA/RHT"
    __buffer = ''
    __buffer2 = ''
    ...
    def dataReceived(self, data):
      ...
      if self.__buffer[0:2] == 'RH=':
          if self.__buffer[-3] != 'C': # i.e. end of dataline <cr><lf>
             # Incomplete; wait for more data
              log.msg('Buffered %s bytes' % len(self.__buffer))
              return
      st = self.__buffer.find('RH=')
      ed = self.__buffer.find('C')
      if ((ed > -1) and (st > -1)):
    ...
    def writedata(self,data):
        ...
        try:
           ...
           self.factory.outfiles[today].flush()
            ...
            try:
                os.umask(022)
                outfile = os.path.join(self.outpath,dt.strftime('%Y%m%d_rht.csv'))
...
def main():
    """This runs the protocol on port 4003"""
    factory = protocol.ServerFactory()
    #factory.protocol = Echo
    reactor.listenTCP(4003,factory)
    reactor.run()
